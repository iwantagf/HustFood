...
- Google dùng OAuth thật. Facebook/Instagram OAuth đang được bỏ khỏi scope triển khai hiện tại.
...
Chạy test helper thuần bằng Node test runner.

```bash
npm run backup:db
```

Tạo file backup MySQL vào thư mục `DB_BACKUP_DIR` hoặc `backups/`. Máy chạy lệnh cần có `mariadb-dump` hoặc `mysqldump`.
...
- `docs/FAILURE_TESTS.md`: checklist test mất mạng, DB fail, form error/loading và backup.
- `scripts/backup-db.mjs`: tạo backup MySQL bằng `mariadb-dump` hoặc `mysqldump`.
...
Endpoint social mô phỏng legacy. Google hiện đi qua OAuth thật tại `/api/auth/google`; Facebook/Instagram không triển khai API trong scope hiện tại.
...
Nếu giỏ hàng có món từ nhiều cửa hàng, API tự tách thành nhiều đơn theo `merchantId`. Với `momo` hoặc `card`, API kiểm tra checksum của payment mock trước khi lưu đơn. Nếu payment mock thất bại, đơn được lưu với trạng thái `payment_retry`. Mỗi đơn mới cũng ghi một dòng `PaymentTransaction` để lưu lịch sử thanh toán.
...
Đơn COD phải được ghi nhận `collect_cod` trước khi chuyển `completed`; action này cập nhật `paymentStatus = paid` và ghi thêm `PaymentTransaction`. Action hoàn tiền ghi thêm transaction loại `refund`. Màn hình `/shipper` dùng GPS trình duyệt để sắp xếp/lọc đơn gần điểm lấy hàng và lưu vị trí cuối của người giao hàng vào đơn.
...
#### `GET /api/reviews?orderId=<id>`

Khách hàng lấy đánh giá đã gửi cho đơn của chính mình.

#### `POST /api/reviews`

Khách hàng gửi đánh giá sau khi đơn ở trạng thái `completed`. API nhận `multipart/form-data` gồm `orderId`, `foodRating`, `shipperRating`, `comment` và tối đa 5 ảnh. Comment quá 1000 ký tự bị từ chối; comment có ngôn từ vi phạm được lưu với `status = hidden` để khách vẫn thấy trong đơn nhưng không hiển thị công khai trên cửa hàng.
...
- `provider`: `credentials`, `google` và endpoint social legacy. Facebook/Instagram không triển khai OAuth thật trong scope hiện tại.
...
### 9.6 `PaymentTransaction`

Lưu lịch sử giao dịch theo đơn: phương thức, provider, transaction id, trạng thái, số tiền, checksum, lỗi thanh toán và loại transaction `payment` hoặc `refund`.

### 9.7 `Review`

Lưu đánh giá sau đơn: điểm món ăn, điểm người giao hàng, bình luận, ảnh upload và `status` (`visible`, `hidden`) để ẩn review vi phạm khỏi trang cửa hàng.
...
- Payment transaction history for mock payment, COD collection and refund action.
- Review/rating after completed orders, image upload and profanity-based hidden review moderation.
...
- Daily database backup script/workflow, Node unit tests and CI lint/test/build workflow.
...
Các phần bị bỏ khỏi scope hiện tại:

- Sentiment analysis không nằm trong scope demo hiện tại.
- Facebook/Instagram OAuth không triển khai API trong scope hiện tại.
...
- Facebook/Instagram OAuth đã được bỏ khỏi scope hiện tại, không có API/callback riêng.
- Workflow backup cần GitHub secret `DATABASE_URL` và MySQL dump client truy cập được database.
